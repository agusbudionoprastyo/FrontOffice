...
<script>
    function checkRoomStatus() {
            ...
            <section class="home" id="home">
                ...
                <div class="home__container container grid">
                    <div class="home__data">
                        ...
                        <h1 class="home__data-title"><?= !empty($data['room']) ? trim(explode(',', $data['fname'])[1]) . '. ' . trim(explode(',', $data['fname'])[0]) : '' ?></b></h1>
        ...
        <script>
               // Constants
                const BASE_URL_WITH_ZERO = 'https://ecard.dafam.cloud/?room=';
                const BASE_URL_WITHOUT_ZERO = 'https://ecard.dafam.cloud/?room=0';
                ...
                async function handleRoomNumberInput(roomNumber) {
                    try {
                        ...
                        // Determine the base URL based on the input
                        let redirectUrl;
                        if (roomNumber.startsWith('0')) {
                            redirectUrl = `${BASE_URL_WITH_ZERO}${roomNumber}`;
                        } else {
                            redirectUrl = `${BASE_URL_WITHOUT_ZERO}`;
                        }
                ...
                async function initialLogic() {
                    if (isFirstVisit()) {
                        ...
                        if (isRoomNumberValid()) {
                            ...
                            // Determine the URL with or without '0' based on the saved room number
                            const expectedUrl = roomNumber.startsWith('0')
                                ? `${BASE_URL_WITH_ZERO}${roomNumber}`
                                : `${BASE_URL_WITHOUT_ZERO}${roomNumber}`;

                            // Check if we are already on the correct URL
                            if (!currentUrl.startsWith(expectedUrl) && roomNumber) {
